<template>
  ...
  >
    <template #header>
        ...
        <p class="title-2xl">Agregar permisos a perfil</p>
        </div>
    </template>
    <template #closeicon>
    ...
    <div class="flex-1 space-y-5">
      <div>
        <div  class="pr-2 pb-4">
          <p class="text-sm">
            Seleccione uno o varios permisos para agregar al perfil
          </p>
        </div>
...
const agregarPermisos = async () => {
  if (v$.value.$invalid) {
    ...
    try {
      ...
      const response = await perfilesService.agregarPermiso(props.idPerfil, body);
