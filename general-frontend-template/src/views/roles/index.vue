<template>
  <div class="w-full flex-1 flex justify-center items-center">
    <div class="w-full min-h-screen flex-1 flex justify-start items-start flex-col bg-background">
      ...
      <div class="w-full flex flex-col space-y-8 place-items-center gap-x-8 px-8 pb-8">
        <template v-for="(item, index) in listaRoles" :key="index">
              ...
              <Accordion class="pa-0 ma-0" expandIcon="pi pi-angle-down" collapseIcon="pi pi-angle-up">
                <AccordionTab class="pa-0 ma-0">
                  <template #header>
                    ...
                    <p class="pa-0 ma-0">
                      <roles-data-table :idRol="item.id" />
                    </p>
                </AccordionTab>
            </Accordion>
            </div>
  ...
  <modal-confirmacion
    ...
    titulo="¿Estás seguro de eliminar este rol?"
...
import ModalRol from "@/views/roles/components/modals/ModalRolComponent.vue";
import RolesDataTable from "@/views/roles/components/RolesDataTableComponent.vue";
...
const listaAcciones = ref([
  { label: 'Actualizar rol', classIcon: 'text-primary', icon: 'edit', accion: (item) => showRol('editar', item) },
  // { label: 'Borrar rol', classIcon: 'text-danger', icon: 'delete', accion: (item) => showEliminar(item) },
]);

function showRol(tipo, item){
  type.value = tipo;
  if(tipo === 'editar'){
    rolObj.value = item;
    rolModal.value = true;
...
const listarRoles = async () => {
  ...
  // Llamar al servicio para obtener los nuevos roles
  const response = await rolService.obtenerRoles();

  // Asignar los nuevos valores a listaRoles
  listaRoles.value = response.data.map(item => ({
    id: item.id,
    nombre: item.nombre,
  }));
...
const eliminarRol = async () => {
  try {
    const response = await rolService.eliminarRol(rolObj.value.id);
    if(response.status == 201 || response.status == 200){
      successToast('Roles', response.data.message)
      rolObj.value = {};
      listarRoles();
    }else{
